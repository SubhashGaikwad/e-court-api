...
class HearingResourceIT {
    ...
    @Test
    @Transactional
    void getAllHearingsByStatusIsEqualToSomething() throws Exception {
        // Initialize the database
        hearingRepository.saveAndFlush(hearing);

        // Get all the hearingList where status equals to DEFAULT_STATUS
        defaultHearingShouldBeFound("status.equals=" + DEFAULT_STATUS);

        // Get all the hearingList where status equals to UPDATED_STATUS
        defaultHearingShouldNotBeFound("status.equals=" + UPDATED_STATUS);
    }

    @Test
    @Transactional
    void getAllHearingsByStatusIsNotEqualToSomething() throws Exception {
        // Initialize the database
        hearingRepository.saveAndFlush(hearing);

        // Get all the hearingList where status not equals to DEFAULT_STATUS
        defaultHearingShouldNotBeFound("status.notEquals=" + DEFAULT_STATUS);

        // Get all the hearingList where status not equals to UPDATED_STATUS
        defaultHearingShouldBeFound("status.notEquals=" + UPDATED_STATUS);
    }

    @Test
    @Transactional
    void getAllHearingsByStatusIsInShouldWork() throws Exception {
        // Initialize the database
        hearingRepository.saveAndFlush(hearing);

        // Get all the hearingList where status in DEFAULT_STATUS or UPDATED_STATUS
        defaultHearingShouldBeFound("status.in=" + DEFAULT_STATUS + "," + UPDATED_STATUS);

        // Get all the hearingList where status equals to UPDATED_STATUS
        defaultHearingShouldNotBeFound("status.in=" + UPDATED_STATUS);
    }

    @Test
    @Transactional
    void getAllHearingsByStatusIsNullOrNotNull() throws Exception {
        // Initialize the database
        hearingRepository.saveAndFlush(hearing);

        // Get all the hearingList where status is not null
        defaultHearingShouldBeFound("status.specified=true");

        // Get all the hearingList where status is null
        defaultHearingShouldNotBeFound("status.specified=false");
    }
